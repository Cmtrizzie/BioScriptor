...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/chat/message", requireAuth, upload.single('file'), async (req: any, res) => {
    try {
          ...
          if (fileType === 'docx') {
            fileAnalysis = {
              ...
              stats: {
                ...
                composition: {},
                wordCount: Math.round(req.file.size / 6),
      ...
      // Parse file context if it exists
      let parsedFileContext = null;
      if (typeof fileContext === 'string') {
        try {
          parsedFileContext = JSON.parse(fileContext);
        } catch (e) {
          console.warn('Failed to parse file context:', e);
        }
      } else if (fileContext) {
        parsedFileContext = fileContext;
      }

      const aiResponse = await processQuery(message, fileAnalysis || undefined, req.user.tier, conversationId, conversationHistory, parsedFileContext);
  ...
  // Promo Code Management Routes (Fixed section)
